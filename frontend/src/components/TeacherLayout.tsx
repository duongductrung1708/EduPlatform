...
export default function TeacherLayout({ children }: { children: React.ReactNode }) {
        ...
        [
          {
            ...
            text: `Tin nhắn lớp: ${data.message}`,
    ...
    const handleSubmissionCreated = (...args: unknown[]) => {
      ...
      setNotifications((prev) =>
        [
          {
            id: crypto.randomUUID(),
            text: `HS nộp bài: ${data?.studentId ?? ''}`,
            ts: new Date().toLocaleTimeString(),
          },
          ...prev,
        ].slice(0, 10),
    ...
    const handleEnrollmentAdded = (...args: unknown[]) => {
      ...
      setNotifications((prev) =>
        [
          {
            id: crypto.randomUUID(),
            text: `${name} đã ghi danh vào môn học`,
            ts: new Date().toLocaleTimeString(),
          },
          ...prev,
        ].slice(0, 20),
    ...
    const handleEnrollmentRemoved = (..._args: unknown[]) => {
      setNotifications((prev) =>
        [
          {
            id: crypto.randomUUID(),
            text: `Một học sinh đã bị hủy ghi danh khỏi môn học`,
            ts: new Date().toLocaleTimeString(),
          },
          ...prev,
        ].slice(0, 20),
    ...
    const handleClassroomAdded = (...args: unknown[]) => {
      ...
      setNotifications((prev) =>
        [
          {
            id: crypto.randomUUID(),
            text: `${name} đã được thêm vào lớp học`,
            ts: new Date().toLocaleTimeString(),
          },
          ...prev,
        ].slice(0, 20),
    ...
    const handleClassroomRemoved = (..._args: unknown[]) => {
      setNotifications((prev) =>
        [
          {
            id: crypto.randomUUID(),
            text: `Một học sinh đã bị xóa khỏi lớp học`,
            ts: new Date().toLocaleTimeString(),
          },
          ...prev,
        ].slice(0, 20),
  ...
  return (
    <Box sx={{ display: 'flex', minHeight: '100vh' }}>
          ...
          >
            ...
            <InputBase
              ...
              onChange={(e) => {
                ...
                const q = next.trim();
                const target = q
                  ? `/teacher/classrooms?q=${encodeURIComponent(q)}`
                  : '/teacher/classrooms';
              ...
              onKeyDown={(e) => {
                if (e.key === 'Enter') {
                  const q = search.trim();
                  navigate(
                    q ? `/teacher/classrooms?q=${encodeURIComponent(q)}` : '/teacher/classrooms',
                  );
                }
              }}
            />
      ...
      <Box
        component="main"
        sx={{ flexGrow: 1, p: { xs: 2, md: 3 }, mt: 8, backgroundColor: 'background.default' }}
      >
        {children}
      </Box>
