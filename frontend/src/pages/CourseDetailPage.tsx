...
export default function CourseDetailPage() {
    ...
    const loadCourseProgress = async () => {
      ...
      try {
        ...
        const entry = progressList.find((item: { courseId: string | { _id?: string; id?: string; toString?: () => string } | unknown; progress?: { completedLessons?: unknown[] } }) => {
          ...
          const courseId =
            typeof course === 'string'
              ? course
              : course?._id || course?.id || (typeof course?.toString === 'function' ? course.toString() : '');
        ...
        if (entry) {
          ...
          if (typeof entry.progress?.percentage === 'number') {
            setProgress(entry.progress.percentage);
          } else {
            const parsed = Number(entry.progress?.percentage);
        ...
        console.error('Không thể tải tiến độ môn học', err);
      }
    };
  ...
  const completedLessonCount = useMemo(() => {
    if (!modules.length || !completedLessons.length) {
      return 0;
    }
    ...
    modules.forEach((module) => {
      (module.lessons || []).forEach((lesson) => {
        if (completedSet.has(String(lesson._id))) {
          count += 1;
        }
      });
    });
  ...
  const derivedProgress = useMemo(() => {
    ...
    return Math.min(100, Math.round((completedLessonCount / totalLessons) * 100));
  ...
  return (
    <>
              ...
              <Stack spacing={2}>
                ...
                {course.createdBy && (
                  <Stack direction="row" spacing={2} alignItems="center">
                    <Avatar sx={{ bgcolor: '#EF5B5B' }}>{course.createdBy.name.charAt(0)}</Avatar>
                    <Box>
                    ...
                    <Stack spacing={2}>
                      ...
                      {user?.role === 'student' && (
                        <Box sx={{ mt: 2, pt: 2, borderTop: 1, borderColor: 'divider' }}>
                          ...
                          {studentRating ? (
                            <Stack spacing={1}>
                              <Rating value={studentRating} readOnly size="small" />
                              {studentReview && (
            ...
            {modules.length > 0 && (
              ...
              >
                ...
                >
                  ...
                  {Array.from(new Set(modules.map((m) => m.volume).filter(Boolean))).map(
                    (volume) => (
                      ...
                      <AccordionSummary expandIcon={<ExpandMoreIcon />}>
                        ...
                        >
                          ...
                          {module.volume && (
                            <Chip
                              label={
                                module.volume.startsWith('Tập') ? module.volume : `${module.volume}`
                              }
                              size="small"
                              variant="outlined"
                        ...
                        <List>
                          {module.lessons.map((lesson) => {
                            const isLessonCompleted =
                              completedLessons.includes(String(lesson._id)) || lesson.isCompleted;
                            return (
                                        ...
                                        <Tooltip title="Xem nhanh">
                                          <IconButton
                                            ...
                                            onClick={() => {
                                              ...
                                              setPreviewTitle(
                                                lesson.content?.fileName || 'Xem tài liệu',
                                              );
                                              setPreviewOpen(true);
                                            }}
      ...
      <Dialog open={previewOpen} onClose={() => setPreviewOpen(false)} fullWidth maxWidth="lg">
        ...
        <DialogContent sx={{ maxHeight: '85vh', overflow: 'auto' }}>
          {previewKind === 'image' && (
            <Box
              sx={{ width: '100%', maxHeight: '85vh', display: 'flex', justifyContent: 'center' }}
            >
              <img
                src={previewUrl}
                alt={previewTitle}
          ...
          {previewKind === 'video' && (
            <Box sx={{ width: '100%', maxHeight: '85vh' }}>
              <video
                src={previewUrl}
                controls
                preload="metadata"
          ...
          {previewKind === 'other' && (
            <Typography variant="body2" color="text.secondary">
              Không hỗ trợ xem nhanh định dạng này. Vui lòng tải về hoặc mở tab mới.
            </Typography>
          )}
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={ratingDialogOpen} onClose={() => setRatingDialogOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <Stack spacing={3} sx={{ mt: 1 }}>
            ...
            <TextField
              ...
              placeholder="Chia sẻ cảm nhận của bạn về môn học này..."
              value={studentReview}
        ...
        <DialogActions sx={{ p: 2 }}>
          ...
          <Button
            ...
            variant="contained"
            disabled={!studentRating || submittingRating}
            sx={{
              background: 'linear-gradient(135deg, #EF5B5B 0%, #FF7B7B 100%)',
              '&:hover': {
                background: 'linear-gradient(135deg, #D94A4A 0%, #EF5B5B 100%)',
